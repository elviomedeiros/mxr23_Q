...
subtitle: |
  Physiological biomarkers of fish from an Atlantic Forest stream
author:
  - name: Adamastor Coutinho Pinto
    ...
    roles: []
    affiliations:
      - Universidade Estadual da Paraíba - Campus V, rua Horácio Trajano, s/n, Cristo Redentor, 58071160, João Pessoa, PB, Brazil.
...
![Total protein (mg) variance of data inside and outside ReBio Guaribas.](imagens/Figura%203.png "Total protein (mg) variance of data inside and outside ReBio Guaribas."){#fig-Boxplot PT In and Out}

![Boxplot with total protein data of collected fish from the sampling sites inside (green) and outside ReBio Guaribas (red), including its respective grouping by similarity above each box.](imagens/Figura%204.png "Boxplot with total protein data of collected fish from the sampling sites inside (green) and outside ReBio Guaribas (red), including its respective grouping by similarity above each box."){#fig-Boxplot PT sites}

![Moisture content (%) of tissues from sampling sites inside and outside the ReBio Guaribas, with the respective p-value between each locality.](imagens/Figura%205.png "Moisture content (%) of tissues from sampling sites inside and outside the ReBio Guaribas, with the respective p-value between each locality."){#fig- MMC sites in and out}

![Boxplot with gill and muscle moisture content data of fish collected from sites inside (green) and outside ReBio Guaribas (red), with its respective grouping by similarity above each box.](imagens/Figura%206.png "Boxplot with gill and muscle moisture content data of fish collected from sites inside (green) and outside ReBio Guaribas (red), with its respective grouping by similarity above each box."){#fig-Boxplot GMC sites}

![Average fluorescence data from sampling sites inside and outside the ReBio Guaribas, expressed by relative fluorescence units/milligrammes of tissue (gill).](imagens/Figura%207.png "Average fluorescence data from sampling sites inside and outside the ReBio Guaribas, expressed by relative fluorescence units/milligrammes of tissue (gill)."){#fig- Boxplot fluorescence}
