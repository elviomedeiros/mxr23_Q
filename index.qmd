...
### selecionando colunas para agrupamento
# habitat
colnames(m_part_cols)
habitat_part <- m_part_cols[, c(
  "w.Temp_C", "w.DO_mg.L", "w.Cond_uS.cm", "w.Turb_ntu", "w.pH",
  "m.Elev_m", "m.Vel_m.s", "m.Slope", "m.Depth_max_cm", "m.Depth_avg_cm",
  "m.Depth_mar_cm", "m.Width_m", "s.Mud", "s.Sand", "s.Gravel_f",
  "s.Rocks", "h.Macrophytes", "h.Grass", "h.Subm_veg", "h.Overh_veg",
  "h.Leaf_l", "h.Algae_f", "h.Algae_a", "h.Root_masses", "h.Debris_l",
  "h.Debris_s"
)]

# nutrientes
colnames(m_part_cols)
nutriente <- m_part_cols[, c(
  "n.Ammonia_NH3_ug.L", "n.Nitrite_NO2__ug.L",
"n.Nitrate_NO3__ug.L", "n.Orthophosphate_SRP__ug.L",
"n.Phosphorus_TP__ug.L"
)]
...
write.table(m_part_cols, "m_amb.csv",
            sep = ";", dec = ".", #"\t",
            row.names = TRUE,
            quote = TRUE,
            append = FALSE)
write.table(habitat_part, "m_hab.csv",
            sep = ";", dec = ".", #"\t",
            row.names = TRUE,
            quote = TRUE,
            append = FALSE)
write.table(nutriente, "m_nutri.csv",
            sep = ";", dec = ".", #"\t",
            row.names = TRUE,
            quote = TRUE,
            append = FALSE)
...
m_hab <- read.csv("m_hab.csv",
                  ...
                  header = TRUE,
                  na.strings = NA)
m_nutri <- read.csv("m_nutri.csv",
                  sep = ";", dec = ".",
                  row.names = 1,
                  header = TRUE,
                  na.strings = NA)
m_amb <- read.csv("m_amb.csv",
                  sep = ";", dec = ".",
                  row.names = 1,
                  header = TRUE,
                  na.strings = NA)
...
del_cols <- c(del_cols, "n.Ammonia_NH3_ug.L", "n.Nitrite_NO2__ug.L",
              "n.Nitrate_NO3__ug.L", "n.Orthophosphate_SRP__ug.L",
              "n.Phosphorus_TP__ug.L")
...
chart.Correlation(m_hab[, c(15:24, 27:31)], histogram=TRUE)
...
#| code-fold: true
#| code-overflow: wrap
#| code-summary: "Code: nutrient table" 
